...
4. **umask**：用来指定目前用户在新建文件或目录时候的权限默认值
   - `umask -S`：查看当前用户在新建文件或目录时候的权限默认值
   - umask的分数指的是，需要减掉的权限。而不是为当前用户增加权限，是减权限。
5. 
...
磁盘的每一面被分为很多条**磁道**，即表面上的一些同心圆，越接近中心，圆就越小。而每一个磁道又按**512个字节**为单位划分为等分，叫做扇区。
...
# First Sector 起始扇区，默认从第2048块扇区开始是因为,由于EFI的兴起，要给EFI 代码留磁盘最开始的1M空间，即2048*512/(1024*1024)=1M
...
**env——查看所有环境变量，或使用printenv**

**在设置完环境变量后，可以通过此命令查看环境变量是否生效。**
