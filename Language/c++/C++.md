...
- 可以使用GNC（GNU Compile Collection）中的gcc和g++去编译C/C++
  - 对于 `*.c和*.cpp`文件，gcc分别当做c和cpp文件编译
  - 对于` *.c和*.cpp`文件，g++则统一当做cpp文件编译
- Clang 是另一个广泛使用的编译器，可以使用clang和clang++去编译C/C++

在Windows系统中：

- MinGW（Minimalist GNU for Windows），里面包含了gcc和g++
- MSVC（Microsoft Visual C++）是 Microsoft 提供的编译器，集成在 Visual Studio 开发环境中。使用 `cl` 命令可以编译 C 和 C++ 

跨平台编译器CMake：

CMake 不是编译器，而是一个构建系统生成工具，常用于跨平台开发。你可以用它生成针对不同系统的构建文件（如 Makefile 或 Visual Studio 项目文件），然后使用适当的编译器进行编译。
...
- 当把指针作为参数进行传递时，也是将实参的一个拷贝传递给形参，两者指向的地址相同，但不是 同一个变量，在函数中改变这个变量的指向（存储的地址）不影响实参，而引用却可以。
...
1. **在类中定义的成员函数默认都是内联函数（不加inline也是内联函数），而在类外定义的成员函数，不默认为inline函数**
...
c++在使用c语言代码的时候出现大量的undefined reference to

原因可能是：[c++调用c，因为主调代码为c++代码，而被调代码为c，在编译的时候，c++在调用c的函数的地方，g++在编译这些函数时，名称会发生变化，所以在库里会找不到对应的函数，这时就会报undefined reference to xxxx](https://stackoverflow.com/questions/55963016/ffmpeg-undefined-references-on-multiple-functions)
...
当我们自己定义了带参数的构造函数时，我们最好是声明不带参数的版本以完成无参的变量初始化，此时编译是不会再自动提供默认的无参版本了。我们可以通过使用关键字default来控制默认构造函数的生成，显式地指示编译器生成该函数的默认版本。
...
**拷贝构造函数的调用时机：旧对象 初始化 新对象 才会调用拷贝构造**
...
1. 所有对象共享同一个static成员函数。
2. static成员函数没有 this 指针，只能访问static成员变量，但不能访问非 static 成员。普通成员函数有 this 指针，可以访问类中的任意成员；
3. static成员函数**既可以在类的内部定义，也可以在类的外部定义**，在外部定义时，不能重复指定static保留字；
...
**如果静态成员变量是私有的（private），我们仍然可以在类的外部进行初始化，这是因为静态成员变量的初始化与它的访问权限是分开的。**但是，除了初始化之外，类的外部代码不能直接访问或修改私有静态成员变量，必须通过类的公共接口（如静态成员函数）来操作它。
...
类的成员函数中，能调用 `delete this`，但在调用这个之后，不能再访问虚函数和对象的数据成员。

delete的本质是“为将被释放的内存调用一个或多个析构函数，然后，释放内存”，`delete this`会去调用本对象的析构函数，而析构函数中又调用`delete this`，形成无限递归，造成堆栈溢出，系统崩溃。
...
1. **友元关系不能被继承。** 友元函数不是作为另一个类的成员，所以不能继承。**并没有破坏继承机制。**
...
typedef void (*PF1)();			// 静态成员函数的类型，和C语言中函数指针的声明差不多
typedef void (ClsFun::*PF2)();	// 非静态成员函数的类型，必须加上类的作用域
// 静态成员函数类型和非静态成员函数的指针类型，无法混用
int main(){
    PF1 pf1= ClsFun::fun1;		// 静态成员函数的地址可以用 取址符，也可以不用
    pf1();
    PF2 pf2 = &ClsFun::fun2;            // 非静态成员函数的地址 必须使用 取址符
    // error: must use '.*' or '->*' to call pointer-to-member function in 'pf2 (...)', e.g. '(... ->* pf2) (...)
    ClsFun af;
    (af.*pf2)();	// 如果想要调用非静态的成员函数，必须通过对象或对象的指针，对成员函数的地址解引用，然后采用调用函数（(xx.*)()，(xx->*)()）
}
```
...
1. 创建派生类对象时，程序先调用基类构造函数，再调用子类成员的构造函数，最后调用派生类构造函数
2. 派生类有的多个基类中有虚基类（虚拟继承的父类）时，优先调用虚基类的构造函数。
3. 派生类有多个基类时，按派生类声明中的基类列表顺序调用构造函数（和子类成员调用顺序规则一致）
4. 派生类构造函数的参数列表后，应当指明基类的构造函数，否则使用基类的默认构造函数；该指明顺序（初始化序列顺序）与基类构造函数调用顺序无关
5. 在多重派生中，由同一个初始基类派生的多个类（都虚继承初始基类）作为派生类的基类时，该初代基类的构造函数只调用一次。如果没有虚拟继承初始基类，那么会初始基类会调用多次。
6. 析构函数顺序，与构造函数调用顺序相反。
...
class G:public H,public D,public C{
public:
    E e;
    F f;
public:
...
**虚函数必须是类的一个成员函数（非全局），不能使友元函数（非友元），也不能是静态的成员函数（非静态）。** 
...
2. Dog继承Animal的时，也会继承虚函数指针和虚函数表， 一旦Dog重写了父类的虚函数，那么此时speak的入口地址就会被替换为Dog::speak。
   - 子类和基类各自有自己虚函数表和虚函数指针，如果子类添加了新的虚函数，那么虚函数表的内容就会增加，如果没有新增虚函数，那么虚函数表将会和基类相同。
   - 在多继承的情况下，子类可以有多个虚函数表指针和多个虚函数表。
...
4. 抽象类主要的目的 是设计 类的接口
5. 抽象类不能用作参数类型、函数返回类型或显式转换的类型，但可以定义抽象类的指针和引用，指针可以指向派生类，进而实现多态。
...
virtual修饰析构函数，虚析构：通过父类指针 释放整个子类空间。

和虚函数的动态绑定原理相似，有点区别就是，找到虚析构函数实际指的是子类的析构函数，子类析构之后，会自动调父类的析构函数。

**构造函数不能是虚函数。**（假设构造函数为虚函数，而虚函数的调用需要虚表，虚表又由构造函数建立。这样就矛盾了）
...
2. 函数模板 自动类型推导时，
   ...
   - 函数模板推导出现逻辑不自洽时，不能对函数的参数进行自动转换。（如果利用自动类型推导，参数不会发生隐式类型转换）

   - 如果强制<>调用时，就支持参数进行自动转换。（如果利用显示指定类型，参数可以发生隐式类型转换 ）
...
静态类型转换，**在编译时进行类型检查。**（检查需要转换的指针变量 声明类型是否支持转换）
...
动态类型转换**在运行时检查类型**（检查实际指针变量实际指向类型是否支持转换）
...
operator[];//返回索引 idx 所指的数据，与C语言兼容，越界不报错。
front();//返回容器中第一个数据元素
back();//返回容器中最后一个数据元素
...
erase(const_iterator pos);//删除迭代器指向的元素， 返回值是一个迭代器，指向删除元素下一个元素。
clear();//删除容器中所有元素
...
```c++
    ...
    // 这里必须将std::async(std::launch::async, func1);的值付给future对象，否则主线程调用者会在此停滞直到func1执行结束，这就相当于一个同步调用。
