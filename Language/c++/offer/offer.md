...
7. 构造函数的种类
   ...
   - 拷贝构造函数
     ...
     - 完成对是整个对象的拷贝，旧对象和复制构造的对象各自占有不同的但同样大小的堆内存。
     - 拷贝构造的形参是一个左值引用。
     - 三种情况被调用：
...
14. TCP和UDP在网络协议的哪一层，二者的区别，头部结构
    ...
    - 在传输层。
    - 连接：TCP有连接，UDP无连接，TCP是一对一的连接，而UDP则可以支持一对一，多对多，一对多的通信。
...
3. 锁升级
   - 指将当前锁的粒度降低，eg：数据库可以把一个表的1000个行锁，升级为一个页锁，或者将页锁升级为表锁。
