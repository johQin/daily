...
- **主设备号**
  - 用于标识驱动程序，一个驱动程序只能有一个主设备号，主设备号一样的设备文件将使用同一类**驱动程序**。
  - 范围：1-254，0有特殊的用途
- **从设备号**
  - 用于标识使用同一驱动程序的不同**具体硬件**和功能。（用于确定设备文件所指的**设备**）
  - 从设备号一般都是用户手动指定分配的
  - 范围：0-255
...
- 模块操作指令
  ...
  - lsmod 列举当前系统中的所有模块（读取proc/modules虚拟文件）
  ...
  - insmod xxx.ko 加载指定模块到内核
    ...
    - modprobe：和insmod类似，如果即将要装载的模块引用了一些当前内核不存在的符号，那么modprobe会在当前模块搜索路径中查找定义了这些符号的其它模块，如果找到了这些模块（即要装载的模块所依赖的模块），它会同时将这些模块装载到内核。如果是在上述这种情况下，使用insmod会失效，并且在系统日志文件中报："unresolved symbols"，insmod会在**内核符号表**中查找符号，如果未找到，则报unresolved symbols
...
如果针对不同的内核版本，我们的驱动程序会发生变化的话，那么我们需要读取内核树中的一个文件（`vermagic.o`），该文件包含了大量有关内核的信息。

如果我们要编写一个能够和多个内核版本在一起工作的模块，则必须使用宏以及#ifdef来构造并编译自己的模块。这些宏放在`linux/version.h`，它被自动包含于`linux/module.h`。

### 内核符号表

公共内核符号表包含了**所有的全局内核项**。

如果一个模块需要向其他模块导出符号，且避免命名污染，则使用下面的宏

```c
EXPORT_SYMBOL(name)
EXPORT_SYMBOL_GPL(name)	// _GPL版本使得要导出的模块只能被GPL许可证下的模块使用
```
