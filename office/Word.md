...
4. 统计当前节的总页码数
   ...
   - 使用 `SECTIONPAGES` 域（最简单）

     这是官方推荐的标准方法，仅适用于**每节都从1开始重新编号**的情形。

     1. **定位**：进入页脚编辑区，将光标放在你想显示“共?页”的位置。
     2. **插入域**：按下键盘上的 **`Ctrl + F9`**，插入一对花括号 `{ }`。
     3. **输入代码**：在花括号内输入 `SECTIONPAGES`。
     4. **更新**：按 `F9` 更新域，就会显示出当前节的页数。

     你的页脚里可以有这样的效果：`第 { PAGE } 页，共 { SECTIONPAGES } 页`。
