...
- 在同一个冲突域中的每个节点都能收到所有被发送的帧——同一时间只能有一台设备发送信息的范围。
- 一次只有一个设备发送信息，其他设备只能等待，否则会发生冲突
- 第二层交换机可解决冲突域的问题

广播域

- 网络中能接收任意设备发出的广播帧的所有设备的集合，——如果站点发出一个广播信号，所有能收到这个信号的设备范围。
- 接收同样广播消息的节点的集合。
- 第三层交换机可解决广播域的问题
...
交换机存储着每个端口相连设备网卡的mac地址，通信的时候，查找目的mac，然后单独连接源端口和目的端口（独占带宽）。

利用以太网可以方便地实现VLAN（VLAN可以阻隔冲突域和广播域）。

交换机**独占传输媒体的带宽（带宽独占，但分时轮转）**，对于普通的10M的共享以太网，若有N个用户，每个用户的平均带宽只有总带宽的1/N。而如果使用交换机时，一个用户在通信时是独占而不是和其他网络用户共享传输媒体带宽。因此拥有N对端口的交换机的总容量为Nx10M。
...
**一个网卡在同一时间内只有一个IP地址，一台电脑可以同时有多个IP地址，每增加一个网络设备即可多增加一个IP地址。**

如常见的笔记本电脑配置有有线网卡和无线网卡，则分别有线网卡有一个IP地址，无线网卡有一个IP地址。
...
- 静态NAT
  - 当内网主机 10.1.1.1如果要与外网的主机201.0.0.11通信时，主机（IP：10.1.1.1）的数据包经过路由器时，路由器通过查找NAT table 将IP数据包的源IP地址（10.1.1.1）改成与之对应的全局IP地址（201.0.0.1），而目标IP地址201.0.0.11保持不变，这样，数据包就能到达201.0.0.11。而当主机HostB(IP:201.0.0.11) 响应的数据包到达与内网相连接的路由器时，路由器同样查找NAT table，将IP数据包的目的IP 地址改成10.1.1.1，这样内网主机就能接收到外网主机发过来的数据包。
  - 在静态NAT方式中，内部的IP地址与公有IP地址是一种**一一对应**的映射关系，所以，采用这种方式的前提是，机构能够申请到足够多的全局IP地址
- 动态NAT
  - 动态地址NAT只是转换IP地址，它为每一个内部的IP地址分配一个临时的外部IP地址，主要应用于拨号，对于频繁的远程联接也可以采用动态NAT。当局域网用户联接上之后，动态地址NAT就会分配给他一个IP地址，用户断开时，这个IP地址就会被释放而留待以后使用。
  - 动态NAT方式适合于 当机构申请到的全局IP地址较少，而内部网络主机较多的情况。内网主机IP与全局IP地址是**多对一**的关系。
- 网络地址端口NAT
  - 网络地址端口转换NAPT（Network Address Port Translation）则是把内部地址映射到外部网络的一个IP地址的不同端口上。
  - 它又包含两种转换方式：SNAT（Source NAT，SNAT）和DNAT（Destination NAT，DNAT）。
  - SNAT： 修改数据包的源地址。源NAT改变第一个数据包的来源地址，它永远会在数据包发送到网络之前完成
    - 当内网主机（10.1.1.3）往外发送数据包时，则需要修改数据包的IP地址和TCP/UDP端口号
      - 源IP：10.1.1.3 源port：1493   --->> 源IP：201.0.0.1， 源port：1492（注意：源端口号可以与原来的一样也可以不一样）
    - 当外网主机（201.0.0.11）响应内网主机（10.1.1.3）时
      - 目的IP：201.0.0.1， 目的port：1492  -->>  目的IP：10.1.1.3  目的port：1493
  - DNAT：修改数据包中的目标IP地址
- 
...
在变长子网掩码的基础上，**CIDR（Classless InterDomain Routing，无分类域间路由）**消除了传统A、B、C类地址及划分子网的概念，将网络前缀都相同的的连续ip地址组成 ” CIDR 地址块“

- ip地址的无分类两级编址为：IP::={<网络前缀>，<主机号>}
- CIDR记法（斜线记法），即：IP地址/网络前缀所占的比特数，eg：128.14.32.5/20，它的掩码是由20个连续的1和后续12个连续的0组成。



### 4.3.5 ARP、DHCP与ICMP

IP地址是网络层使用的地址，MAC地址是数据链路层使用的地址。数据分组封装成帧后，数据链路层是看不见 数据分组中的IP地址的。

由于物理地址随设备的更换而变更。

**ARP（Address Resolution Protocol，地址解析协议）**：用于解决同一局域网上主机或路由器的**IP地址和MAC地址的映射问题**（通过IP地址寻找MAC地址），RARP：MAC地址找IP

**DHCP（Dynamic Host Configration Protocol，动态主机配置协议）**：**常用于给主机动态地分配IP地址**，它提供了即插即用联网机制，这种机制允许一台计算机加入新的网络和自动获取IP地址。DHCP是应用层协议，基于UDP。

**ICMP（Internet Control Message Protocol，网际控制报文协议）**：用于目标主机或到目标主机的路径上的路由器向源主机报告差错和异常情况

- 常见应用：PING（用来测试两主机之间的连通性，工作在应用层，直接使用的ICMP，没有经过TCP/UDP），TracerRoute/Tracer（用来跟踪分组经过的路由，工作在网络层）
...
- 静态路由算法（非自适应）：指由网络管理员手工配置的路由信息
- 动态路由算法（自适应）：指路由器的路由表项是通过相互连接的路由器之间彼此交换信息，然后按一定的算法游湖出来的。

常用的动态路由算法可分为两类：

- 距离（跳数）-向量算法：
  - 所有结点**定期**将他们的整个路由选择表传送给所有与之**直接相邻**（物理相连）的结点。
  - 典型算法：RIP
- 链路状态算法：
  - 要求参与该算法的所有路由掌握完整的网络拓扑和链路开销。
  - 典型算法：OSPF

#### 层次路由

当网络规模扩大时，路由器的路由表成比例地增大，相当耗费路由器缓存空间，也会消耗更多带宽去传递路由信息

因此路由选择必须按照层次的方式进行。因特网将整个互联网划分为许多较小的自治系统。

自治系统（Autonomous System，自治域）：单一技术管理下的一组路由器，这些路由器在AS内部使用同一种路由选择协议，在AS之间也使用同一种路由协议。

因特网将路由选择协议划分为两大类：

- 内部网关协议（IGP)：也称域内路由选择，一个自治系统内部所使用的路由选择协议，具体协议：RIP、OSPF
- 外部网关协议（EGP）：也称域间路由选择，自治系统之间所使用的路由选择协议，用在不同自治系统的路由器之间交换路由信息，并负责为分组在不同AS之间选择最优路径。具体协议：BGP
...
路由器是一种具有多个输入和输出端口的专用计算机。用于连接不同的网络（异构）并完成路由转发

路由器由于互联多个网络，因此它不仅有多个IP地址，也有多个硬件地址（有多个网卡）。
...
| 应用程序   | FTP数据链接 | FTP控制链接 | SSH  | TELNET | SMTP | DNS  | TFTP | HTTP | POP3 | SNMP |
| ---------- | ----------- | ----------- | ---- | ------ | ---- | ---- | ---- | ---- | ---- | ---- |
| 传输层协议 | TCP         | TCP         | TCP  | TCP    | TCP  | UDP  | UDP  | TCP  | TCP  | UDP  |
| 端口号     | 80          | 21          | 22   | 23     | 25   | 53   | 69   | 80   | 110  | 161  |
